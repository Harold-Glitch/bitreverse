...
namespace secp256k1 {
	...
	public:
		...
		uint256(uint8_t *x, int endian = LittleEndian)
		{
			if (endian == LittleEndian) {
				memcpy(this->v, x, sizeof(this->v));
			}
			else {
				uint8_t buf[32];
				for (int i = 0; i < 32; i++)
				...
				memcpy(this->v, buf, sizeof(this->v));
			}
		}
		...
		void exportWords(uint32_t *buf, int len, int endian = LittleEndian) const
		{
			if(endian == LittleEndian) {
				for(int i = 0; i < len; i++) {
					buf[i] = v[i];
		...
		void toByteArray(uint8_t *buf) const
		{
			uint32_t uia[8];
			for (int i = 0; i < 8; i++)
				uia[8 - i - 1] = v[i];
			...
			for (int i = 0; i < 8; i++) 
				uia[i] = (uia[i] << 24) | ((uia[i] << 8) & 0x00ff0000) | ((uia[i] >> 8) & 0x0000ff00) | (uia[i] >> 24);

			memcpy(buf, (uint8_t *)uia, 32);
		}
		...
		uint256 add(uint32_t val) const;
	...
	const uint32_t _POINT_AT_INFINITY_WORDS[8] = { 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF };
	const uint32_t _P_WORDS[8] = { 0xFFFFFC2F, 0xFFFFFFFE, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF };
	const uint32_t _N_WORDS[8] = { 0xD0364141, 0xBFD25E8C, 0xAF48A03B, 0xBAAEDCE6, 0xFFFFFFFE, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF };
	const uint32_t _GX_WORDS[8] = { 0x16F81798, 0x59F2815B, 0x2DCE28D9, 0x029BFCDB, 0xCE870B07, 0x55A06295, 0xF9DCBBAC, 0x79BE667E };
	const uint32_t _GY_WORDS[8] = { 0xFB10D4B8, 0x9C47D08F, 0xA6855419, 0xFD17B448, 0x0E1108A8, 0x5DA4FBFC, 0x26A3C465, 0x483ADA77 };
	const uint32_t _BETA_WORDS[8] = {0x719501EE, 0xC1396C28, 0x12F58995, 0x9CF04975, 0xAC3434E9, 0x6E64479E, 0x657C0710, 0x7AE96A2B };
	const uint32_t _LAMBDA_WORDS[8] = {0x1B23BD72, 0xDF02967C, 0x20816678, 0x122E22EA, 0x8812645A, 0xA5261C02, 0xC05C30E0, 0x5363AD4C };
