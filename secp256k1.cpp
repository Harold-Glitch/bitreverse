...
static inline void addc(uint32_t a, uint32_t b, uint32_t carryIn, uint32_t &sum, int &carryOut)
{
...
static inline void subc(uint32_t a, uint32_t b, uint32_t borrowIn, uint32_t &diff, int &borrowOut)
{
...
static bool lessThanEqualTo(const uint32_t *a, const uint32_t *b, int len)
{
...
static bool greaterThanEqualTo(const uint32_t *a, const uint32_t *b, int len)
{
	for(int i = len - 1; i >= 0; i--) {
		if(a[i] > b[i]) {
			// is greater than
...
static int add(const uint32_t *a, const uint32_t *b, uint32_t *c, int len)
{
	int carry = 0;
...
static int sub(const uint32_t *a, const uint32_t *b, uint32_t *c, int len)
{
	int borrow = 0;
...
uint256 uint256::mul(int i) const
{
	...
	multiply((uint32_t *)&i, 1, this->v, 8, product);
	...
	while(!isOne(u) && !isOne(v)) {
		...
		while(isEven(u)) {
			...
			if(isEven(x1)) {
				...
				x1.v[7] |= ((uint32_t)x1Signed & 0x01) << 31;
		...
		while(isEven(v)) {
			...
			if(isEven(x2)) {
				...
				x2.v[7] |= ((uint32_t)x2Signed & 0x01) << 31;
...
{
	uint32_t barrettN[] = { 0x2fc9bec0, 0x402da173, 0x50b75fc4, 0x45512319, 0x00000001, 0x00000000, 0x00000000, 0x00000000, 00000001 };
	uint32_t product[25] = { 0 };
...
uint256 generatePrivateKey()
{
	...
	for(int i = 0; i < 8; i++) {
		k.v[i] = ((uint32_t)rand() | ((uint32_t)rand()) << 17);
	}
